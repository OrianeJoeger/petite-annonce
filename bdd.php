...
function createTableUser() {
    $pdo = connexion();
    $requeteSQLCreation = 'CREATE TABLE "users" (
        "id" INTEGER PRIMARY KEY AUTOINCREMENT,
        "name" TEXT,
        "email" TEXT,
        "password" TEXT
...
// création une table au bon format, pour contenir les annonces
function insertUserExemple() {
    $pdo = connexion();
    $requeteSQLCreation = "INSERT INTO users (name, email, password) VALUES ('demo', 'demo@demo.com', 'demo');";
    // exécution de la requête
    $pdo->query($requeteSQLCreation);
}
